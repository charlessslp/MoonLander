...
class Network(nn.Module):
  ...
  def __init__(self, state_size, action_size, seed = 43):
      super(Network, self).__init__()
      self.seed = torch.manual_seed(seed)
      self.fc1 = nn.Linear(state_size, 64)  # We're creating the first connection, from input to 1st hidden layer. 64 is a magic number, decided until trial and error as the optimal number of nodes
      self.fc2 = nn.Linear(64, 64)          # connect the 1st hidden layer to a 2nd hidden layer of another 64 neurons. Again, this is decided as per trial and error as the optimal solution
      self.fc3 = nn.Linear(64, action_size) # finally, connect the 2nd layer to the outputs (aka, Q-values of possible actions to take)
...
class ReplayMemory(object):
  ...
  def push(self, event): # adding the current experience that happened (with its state, action, reward...)
    self.memory.append(event)
    if len(self.memory) > self.capacity:
      del self.memory[0]

  def sample(self, batch_size): # return a batch of experiences, separated in their corresponding arrays, in other words, from the memory, extract a random batch of experiences and then separate the states, actions, rewards etc into separate arrays of each.
...
for episode in range(0, 5):
    state, _ = env.reset()
    frames = compute_video_of_model(agent, 'LunarLander-v3', frames)
...
for episode in range(1, number_episodes + 1):
  ...
  if np.mean(scores_on_100_episodes) >= 400.0:
    print('\n Enviroment solved in {:d} episodes!\tAverage Score: {:.2f}'.format(episode - 100, np.mean(scores_on_100_episodes)))
    break
